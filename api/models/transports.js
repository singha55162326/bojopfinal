...
class Transports {
    ...
    static findTransById(transport_id) {
        let sql = `SELECT tt.transport_id, td.id_documents, td.address,td.contact,td.company,tt.dimention,toi.oils_id,toi.oil_name,tdd.Qty,tdd.Price,ts.supplier_name,tc.car_name,
        tu.username FROM tb_transports tt, tb_documents td, tb_orders tod, tb_oils toi, tb_suppliers ts, tb_cars tc, tb_users tu, tb_orderdetail tdd
        WHERE tt.id_documents = td.id_documents
        AND tt.order_id = tod.orders_id
        AND tt.oil_id = toi.oils_id
        AND tt.supplier_id = ts.supplier_id
        AND tt.car_id = tc.car_id
        AND tt.user_id = tu.user_id
        AND tod.orders_id = tdd.oils_id
