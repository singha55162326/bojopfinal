...
class Documents {
    ...
    static findDocById(id_documents) {
        let sql = `SELECT td.id_documents, td.address,td.order_id,td.contact,td.traid,td.company,td.dimention,td.phone,td.service,
        tu.username,tos.oils_id,tos.oil_name,tod.Qty,tod.Price
        FROM tb_documents td, tb_orders toi, tb_users tu, tb_orderdetail tod , tb_oils tos
        WHERE td.order_id = toi.orders_id
        AND td.user_id = tu.user_id
        AND toi.orders_id = tod.orders_id
        AND tod.oils_id = tos.oils_id 
